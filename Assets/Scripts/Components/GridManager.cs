...
namespace Components
{
    public partial class GridManager : SerializedMonoBehaviour, ITweenContainerBind
    {
        ...
        private void RegisterEvents()
        {
            ...
            GameMenuEvents.GameOver += OnGameOver;
        }
        ...
        private IEnumerator DestroyRoutine()
        {
            foreach(List<Tile> matches in _lastMatches)
            {
                ...
                ShowParticleEffect(_mySettings.MatchParticlePrefab, matches);
                
                GridEvents.MatchGroupDespawn?.Invoke(matches.Count * _scoreMulti);
                
                GameMenuEvents.IncreaseTime(1f);
        ...
        private void DespawnTile(Tile tile) //this was "Tile e"
        {
            ...
            {
                GridEvents.PowerupTileDespawned?.Invoke();
            ...
            GridEvents.NormalTileDespawned?.Invoke();
        ...
        private bool ShouldSpawnPowerup(out int powerupId)
        {
            ...
            if (_scoreMulti == EnvVar.horizontalPowerupThreshold && !_horizontalPowerupPresent)
            {
                powerupId = _mySettings.PowerupPrefabIds[0];
                _horizontalPowerupPresent = true;
                return true;
            }
            if (_scoreMulti == EnvVar.bombPowerupThreshold && !_bombPowerupPresent)
            {
                powerupId = _mySettings.PowerupPrefabIds[1];
                _bombPowerupPresent = true;
                return true;
            }
            if (_scoreMulti == EnvVar.verticalPowerupThreshold && !_verticalPowerupPresent)
            {
        ...
        private void ShowParticleEffect(GameObject particlePrefab, List<Tile> tiles)
        {
            foreach (Tile tile in tiles)
            {
                Instantiate(particlePrefab, tile.transform.position, Quaternion.identity);
            }
        }
        ...
        private void ActivatePowerup(Tile powerupTile)
        {
            ...
            if (powerupTile.ID == _mySettings.PowerupPrefabIds[0])
            {
                for (int x = 0; x < _gridSizeX; x++)
                {
                    ...
                    ShowParticleEffect( _mySettings.HorizontalParticlePrefab, tilesToDestroy);
                }
                _horizontalPowerupPresent = false;
            }
            else if (powerupTile.ID == _mySettings.PowerupPrefabIds[2]) 
            {
                for (int y = 0; y < _gridSizeY; y++)
                {
                    ...
                    ShowParticleEffect(_mySettings.VerticalParticlePrefab, tilesToDestroy);
                }
                _verticalPowerupPresent = false;
            }
            else if (powerupTile.ID == _mySettings.PowerupPrefabIds[1]) 
            {
                ...
                foreach (Vector2Int dir in adjacentDirections)
                {
                    ...
                    if (_grid.IsInsideGrid(adjacentCoord))
                    {
                        ...
                        ShowParticleEffect(_mySettings.BombParticlePrefab, tilesToDestroy);
                    }
                }
                _bombPowerupPresent = false;
            }
            ...
            GameMenuEvents.IncreaseTime(4f);

            bool isVibrationEnabled = PlayerPrefs.GetInt(EnvVar.VibrationPrefKey, 1) == 1;
            if (isVibrationEnabled)
            {
                Handheld.Vibrate();
            }
            
            if (powerupTile.ID == _mySettings.PowerupPrefabIds[0]) _horizontalPowerupPresent = false;
            else if (powerupTile.ID == _mySettings.PowerupPrefabIds[1]) _bombPowerupPresent = false;
        ...
        public class Settings
        {
            ...
            public GameObject MatchParticlePrefab => _matchParticlePrefab;
            ...
            public GameObject VerticalParticlePrefab => _verticalParticlePrefab;
            [SerializeField] private GameObject _verticalParticlePrefab;
            public GameObject HorizontalParticlePrefab => _horizontalParticlePrefab;
            [SerializeField] private GameObject _horizontalParticlePrefab;
        }
    }
} 